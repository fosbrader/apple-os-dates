  ...
  return active
    .map((release): ReleaseForecast => {
      ...
      if (daysSinceLatestMilestone > FORECAST_STALE_AFTER_DAYS) {
        ...
        statusMessage =
          "Forecast paused: the expected next milestone window passed without a recorded milestone. This is anomalous against recent comparable Apple release cycles, though rare historical exceptions exist; it does not mean the release is cancelled or that the public release is late. A newer milestone is needed before publishing another estimate.";
      }
