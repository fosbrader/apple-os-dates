import {
  ...
  type ForecastBenchmarkIdV1,
  type ForecastBenchmarkUnavailableReasonV1,
...
/** Each available target carries the closed three-row origin benchmark set. */
export const FORECAST_SHADOW_EPOCH_MAX_BENCHMARK_ROWS_PER_FORECAST =
  FORECAST_SHADOW_EPOCH_MAX_TARGETS_PER_FORECAST * FORECAST_BENCHMARK_IDS.length;
...
export const FORECAST_SCORE_POINT_ESTIMATORS = [
  "platform-stage-median",
  "hierarchical-platform-cadence",
  "next-event-timing-median",
  "current-public-heuristic",
  "next-event-simple-baseline",
] as const;
export type ForecastScorePointEstimatorV1 =
  (typeof FORECAST_SCORE_POINT_ESTIMATORS)[number];
...
export interface ForecastScoreArtifactV1 {
  ...
  forecastBenchmarkFingerprint: string;
  forecastBenchmarkSourceFingerprint: string;
  targetId: string;
  targetKind: ForecastTargetKind;
  releaseId: string;
  platformId: string;
  ...
  actualDays: number;
  pointEstimator: ForecastScorePointEstimatorV1;
  pointDays: number;
  signedErrorDays: number;
  absoluteErrorDays: number;
  /** Only intervals frozen by this exact benchmark are scored. */
  intervals: readonly ForecastScoreIntervalV1[];
}
...
/**
 * An immutable benchmark row that was unavailable at forecast origin. These
 * rows preserve denominator semantics without treating an absent or
 * incomparable comparator as a pending outcome or a data-quality defect.
 */
export interface ForecastReconciliationUnavailableBenchmarkEntryV1
  extends ForecastReconciliationEntryBaseV1 {
...
export interface ForecastReconciliationIndexV1 {
  ...
  unavailableBenchmarks: readonly ForecastReconciliationUnavailableBenchmarkEntryV1[];
...
export interface ForecastShadowHealthMetricV1Base {
  ...
  coverage50EventCount: number;
  coverage80EventCount: number;
}
...
export interface ForecastShadowHealthReportV1 {
  ...
  summary: {
    /** Available benchmark predictions only; unavailable rows are separate. */
    forecastCount: number;
    unavailableBenchmarkCount: number;
    scoredCount: number;
    pendingCount: number;
    dataGapCount: number;
...
const SCORE_POINT_ESTIMATORS = new Set<ForecastScorePointEstimatorV1>(
  FORECAST_SCORE_POINT_ESTIMATORS,
);
...
function expectedScoreModelVersion(
  targetKind: unknown,
  benchmarkId: unknown,
): string | null {
  if (
    !["public-release", "next-eligible-prerelease-event"].includes(
      targetKind as string,
    ) ||
    !BENCHMARK_IDS.has(benchmarkId as ForecastBenchmarkIdV1)
  ) {
    return null;
  }
  if (benchmarkId === "current-public-heuristic") {
    return targetKind === "public-release"
      ? FORECAST_CURRENT_PUBLIC_HEURISTIC_VERSION
      : null;
  }
  if (benchmarkId === "simple-baseline") {
    return targetKind === "public-release"
...
function statusKey(value: Pick<ForecastReconciliationEntryBaseV1, "forecastArtifactId" | "benchmarkId" | "targetKind" | "targetId">): string { return `${value.forecastArtifactId}\u0000${value.targetKind}\u0000${value.targetId}\u0000${value.benchmarkId}`; }
...
function metricKey(value: ForecastShadowHealthMetricV1): string { return `${value.targetKind}\u0000${value.benchmarkId}\u0000${value.groupKind}\u0000${value.groupId}`; }
...
export function validateForecastScoreArtifact(value: unknown): ForecastScoringValidationIssue[] {
  try {
    ...
    const issues: ForecastScoringValidationIssue[] = [];
    exactKeys(value, ["scoreVersion", "mode", "forecastArtifactId", "forecastRunKey", "forecastSemanticFingerprint", "forecastGeneratedAt", "forecastDataCutoff", "sourceDatasetVersion", "sourceDatasetFingerprint", "sourceOutcomeFingerprint", "benchmarkId", "forecastBenchmarkFingerprint", "forecastBenchmarkSourceFingerprint", "targetId", "targetKind", "releaseId", "platformId", "anchorEventId", "anchorStage", "anchorOccurredOn", "originOn", "modelVersion", "modelFingerprint", "calibrationFingerprint", "modelCohortId", "predictedEligibleStage", "outcomeId", "outcomeEventId", "outcomeStage", "outcomeOccurredOn", "outcomeFirstObservedOn", "outcomeFirstObservedAt", "anchorSourceEvidenceIds", "outcomeSourceEvidenceIds", "actualDays", "pointEstimator", "pointDays", "signedErrorDays", "absoluteErrorDays", "intervals"], "score", issues);
    ...
    for (const field of ["forecastArtifactId", "forecastRunKey", "forecastSemanticFingerprint", "sourceDatasetFingerprint", "sourceOutcomeFingerprint", "forecastBenchmarkFingerprint", "forecastBenchmarkSourceFingerprint", "modelFingerprint"] as const) if (!isSha(value[field])) issues.push({ code: "invalid-fingerprint", path: `score.${field}`, message: "An exact SHA-256 fingerprint is required." });
    if (!(value.calibrationFingerprint === null || isSha(value.calibrationFingerprint))) issues.push({ code: "invalid-fingerprint", path: "score.calibrationFingerprint", message: "Calibration must be an exact SHA-256 fingerprint or null for an uncalibrated benchmark." });
    ...
    if (!["public-release", "next-eligible-prerelease-event"].includes(value.targetKind as string) || !BENCHMARK_IDS.has(value.benchmarkId as ForecastBenchmarkIdV1) || !isText(value.targetId) || !isText(value.releaseId) || !isText(value.platformId) || !isText(value.anchorEventId) || !isText(value.modelVersion) || !isText(value.modelCohortId) || !isText(value.outcomeId) || !isText(value.outcomeEventId)) issues.push({ code: "invalid-row", path: "score", message: "Stable score, benchmark, model, and cohort identities are required." });
    const expectedModelVersion = expectedScoreModelVersion(
      value.targetKind,
      value.benchmarkId,
    );
    if (
      ...
      (value.benchmarkId === "selected-private-model"
        ? !isSha(value.calibrationFingerprint)
        : value.calibrationFingerprint !== null)
    ) {
      issues.push({ code: "invalid-row", path: "score", message: "Benchmark model and calibration identity must match the closed origin contract." });
    }
    ...
    const estimatorIsValid = SCORE_POINT_ESTIMATORS.has(
      value.pointEstimator as ForecastScorePointEstimatorV1,
    ) && (
      value.benchmarkId === "selected-private-model"
        ? (value.targetKind === "public-release"
        ...
        : value.benchmarkId === "current-public-heuristic"
          ? value.targetKind === "public-release" && value.pointEstimator === "current-public-heuristic"
          : value.targetKind === "public-release"
            ? value.pointEstimator === "platform-stage-median"
            : value.pointEstimator === "next-event-simple-baseline"
    );
    ...
    if (!Array.isArray(value.intervals) || value.intervals.length > 2 || !isFiniteNumber(value.actualDays)) issues.push({ code: "invalid-row", path: "score.intervals", message: "A benchmark may retain only its frozen 50% and/or 80% intervals." });
    else {
      const levels = value.intervals.map((interval) => isRecord(interval) ? interval.level : null);
      if (stableSerializeHistoricalAnalysis(levels) !== stableSerializeHistoricalAnalysis([...levels].sort((left, right) => Number(left) - Number(right))) || new Set(levels).size !== levels.length || levels.some((level) => level !== 0.5 && level !== 0.8)) issues.push({ code: "invalid-order", path: "score.intervals", message: "Benchmark intervals must be unique, known levels in canonical order." });
      value.intervals.forEach((interval, index) => {
        if (isRecord(interval) && (interval.level === 0.5 || interval.level === 0.8)) issues.push(...intervalIssues(interval, `score.intervals[${index}]`, interval.level, value.actualDays as number));
        else issues.push({ code: "invalid-row", path: `score.intervals[${index}]`, message: "Each benchmark interval needs an exact supported level." });
      });
      const fifty = value.intervals.find((interval) => isRecord(interval) && interval.level === 0.5);
      const eighty = value.intervals.find((interval) => isRecord(interval) && interval.level === 0.8);
      ...
      if (
        value.benchmarkId === "selected-private-model" &&
        stableSerializeHistoricalAnalysis(levels) !==
          stableSerializeHistoricalAnalysis([0.5, 0.8])
      ) {
        issues.push({ code: "invalid-row", path: "score.intervals", message: "The selected model must retain both frozen calibrated interval levels." });
      }
      if (
        value.benchmarkId !== "selected-private-model" &&
        levels.includes(0.8)
      ) {
        issues.push({ code: "invalid-row", path: "score.intervals", message: "Comparator rows cannot claim an unfrozen 80% interval." });
      }
    }
...
function entryBaseIssues(value: Record<string, unknown>, path: string, issues: ForecastScoringValidationIssue[]): void {
  ...
  exactKeys(snapshot, ["forecastTargetFingerprint", "forecastBenchmarkFingerprint", "forecastDataCutoff", "anchorEventId", "anchorStage", "anchorOccurredOn", "originOn", "anchorSourceEvidenceIds", "predictedEligibleStage"], `${path}.targetSnapshot`, issues);
  if (!isSha(snapshot.forecastTargetFingerprint) || !isSha(snapshot.forecastBenchmarkFingerprint) || !isDay(snapshot.forecastDataCutoff) || !isText(snapshot.anchorEventId) || typeof snapshot.anchorStage !== "string" || !CANONICAL_STAGE.test(snapshot.anchorStage) || !isDay(snapshot.anchorOccurredOn) || !isDay(snapshot.originOn) || snapshot.anchorOccurredOn > snapshot.originOn || snapshot.originOn > snapshot.forecastDataCutoff) issues.push({ code: "invalid-row", path: `${path}.targetSnapshot`, message: "The target snapshot must bind exact source-cutoff-bounded target and benchmark chronology." });
  ...
  if (value.targetKind === "next-eligible-prerelease-event" && !(snapshot.predictedEligibleStage === null || ["developer-beta", "public-beta", "release-candidate"].includes(snapshot.predictedEligibleStage as string))) issues.push({ code: "invalid-row", path: `${path}.targetSnapshot.predictedEligibleStage`, message: "Next-event benchmark snapshots require an eligible stage family or explicit null when unavailable." });
}
...
function scoreEntryIssues(value: unknown, path: string): ForecastScoringValidationIssue[] {
  ...
  const issues: ForecastScoringValidationIssue[] = [];
  exactKeys(value, ["forecastArtifactId", "forecastGeneratedAt", "benchmarkId", "targetId", "targetKind", "releaseId", "platformId", "modelCohortId", "targetSnapshot", "scoreArtifactId", "outcomeId", "sourceDatasetFingerprint", "sourceRowFingerprint", "outcomeEventId", "outcomeStage", "outcomeOccurredOn", "outcomeFirstObservedOn", "outcomeFirstObservedAt", "outcomeSourceEvidenceIds", "actualDays", "pointDays", "signedErrorDays", "absoluteErrorDays", "coverage50", "coverage80"], path, issues);
  entryBaseIssues(value, path, issues);
  if (!isSha(value.scoreArtifactId) || !isSha(value.outcomeId) || !isSha(value.sourceDatasetFingerprint) || !isSha(value.sourceRowFingerprint) || !isText(value.outcomeEventId) || typeof value.outcomeStage !== "string" || !CANONICAL_STAGE.test(value.outcomeStage) || !isDay(value.outcomeOccurredOn) || !isDay(value.outcomeFirstObservedOn) || !isInstant(value.outcomeFirstObservedAt) || (isDay(value.outcomeFirstObservedOn) && isInstant(value.outcomeFirstObservedAt) && value.outcomeFirstObservedAt.slice(0, 10) !== value.outcomeFirstObservedOn) || !Number.isSafeInteger(value.actualDays) || (value.actualDays as number) < 1 || !isFiniteNumber(value.pointDays) || (value.pointDays as number) < 0 || !isFiniteNumber(value.signedErrorDays) || !isFiniteNumber(value.absoluteErrorDays) || !(value.coverage50 === null || typeof value.coverage50 === "boolean") || !(value.coverage80 === null || typeof value.coverage80 === "boolean")) issues.push({ code: "invalid-row", path, message: "A score row requires exact source/outcome identity and finite scoring projections." });
  ...
  if (value.targetKind === "next-eligible-prerelease-event" && snapshot && (snapshot.predictedEligibleStage === null || (typeof value.outcomeStage === "string" && value.outcomeStage.split(":")[0] !== snapshot.predictedEligibleStage))) issues.push({ code: "invalid-row", path: `${path}.outcomeStage`, message: "Next-event score rows must match the frozen predicted eligible stage family." });
...
function pendingEntryIssues(value: unknown, path: string): ForecastScoringValidationIssue[] {
  ...
  const issues: ForecastScoringValidationIssue[] = [];
  exactKeys(value, ["forecastArtifactId", "forecastGeneratedAt", "benchmarkId", "targetId", "targetKind", "releaseId", "platformId", "modelCohortId", "targetSnapshot", "reason"], path, issues);
  entryBaseIssues(value, path, issues);
...
function unavailableBenchmarkEntryIssues(
  value: unknown,
  path: string,
): ForecastScoringValidationIssue[] {
  if (!isRecord(value)) {
    return [{ code: "invalid-row", path, message: "Unavailable benchmark entry must be an object." }];
  }
  const issues: ForecastScoringValidationIssue[] = [];
  exactKeys(value, ["forecastArtifactId", "forecastGeneratedAt", "benchmarkId", "targetId", "targetKind", "releaseId", "platformId", "modelCohortId", "targetSnapshot", "reason"], path, issues);
  entryBaseIssues(value, path, issues);
  const reasons = new Set<ForecastBenchmarkUnavailableReasonV1>([
  ...
  if (!reasons.has(value.reason as ForecastBenchmarkUnavailableReasonV1)) {
    issues.push({ code: "invalid-row", path: `${path}.reason`, message: "Unavailable benchmarks require one closed origin-time reason." });
  }
  if (
    value.targetKind === "next-eligible-prerelease-event" &&
    value.benchmarkId === "current-public-heuristic" &&
    value.reason !== "incomparable-target-definition"
  ) {
    issues.push({ code: "incompatible-artifact", path: `${path}.reason`, message: "The current heuristic next-event benchmark is explicitly incomparable." });
  }
  return issues;
}
...
function gapEntryIssues(value: unknown, path: string): ForecastScoringValidationIssue[] {
  ...
  const issues: ForecastScoringValidationIssue[] = [];
  exactKeys(value, ["forecastArtifactId", "forecastGeneratedAt", "benchmarkId", "targetId", "targetKind", "releaseId", "platformId", "modelCohortId", "targetSnapshot", "gapId", "outcomeId", "reason", "sourceEvidenceIds"], path, issues);
  entryBaseIssues(value, path, issues);
...
function auditEntryIssues(value: unknown, path: string): ForecastScoringValidationIssue[] {
  ...
  const issues: ForecastScoringValidationIssue[] = [];
  exactKeys(value, ["forecastArtifactId", "forecastGeneratedAt", "benchmarkId", "targetId", "targetKind", "releaseId", "platformId", "modelCohortId", "targetSnapshot", "auditId", "reason", "previousScoreArtifactId", "previousOutcomeFingerprint", "replacementScoreArtifactId", "replacementOutcomeFingerprint", "sourceDatasetFingerprint", "sourceEvidenceIds"], path, issues);
  entryBaseIssues(value, path, issues);
...
function indexWithoutFingerprint(value: ForecastReconciliationIndexV1): Omit<ForecastReconciliationIndexV1, "indexFingerprint"> {
  return { indexVersion: value.indexVersion, mode: value.mode, compatibleForecastArtifactVersion: value.compatibleForecastArtifactVersion, compatibleScoreVersion: value.compatibleScoreVersion, reconciliationCutoffAt: value.reconciliationCutoffAt, reconciliationCutoffDate: value.reconciliationCutoffDate, evaluationEpoch: value.evaluationEpoch, epochStopReason: value.epochStopReason, sourceForecastArtifactIds: value.sourceForecastArtifactIds, sourceForecasts: value.sourceForecasts, scores: value.scores, pending: value.pending, dataGaps: value.dataGaps, unavailableBenchmarks: value.unavailableBenchmarks, audit: value.audit };
}
...
export function validateForecastReconciliationIndex(value: unknown, scoreArtifacts?: ReadonlyMap<string, ForecastScoreArtifactV1>, forecastArtifacts?: ReadonlyMap<string, ForecastArtifactV1>): ForecastScoringValidationIssue[] {
  try {
    ...
    const issues: ForecastScoringValidationIssue[] = [];
    exactKeys(value, ["indexVersion", "mode", "compatibleForecastArtifactVersion", "compatibleScoreVersion", "reconciliationCutoffAt", "reconciliationCutoffDate", "evaluationEpoch", "epochStopReason", "sourceForecastArtifactIds", "sourceForecasts", "scores", "pending", "dataGaps", "unavailableBenchmarks", "audit", "indexFingerprint"], "index", issues);
    ...
    if (!Array.isArray(value.sourceForecasts) || !Array.isArray(value.scores) || !Array.isArray(value.pending) || !Array.isArray(value.dataGaps) || !Array.isArray(value.unavailableBenchmarks) || !Array.isArray(value.audit)) return [...issues, { code: "invalid-row", path: "index", message: "Source-forecast, score, pending, data-gap, unavailable-benchmark, and audit arrays are required." }];
    ...
    if (value.scores.length > FORECAST_RECONCILIATION_MAX_ROWS || value.pending.length > FORECAST_RECONCILIATION_MAX_ROWS || value.dataGaps.length > FORECAST_RECONCILIATION_MAX_ROWS || value.unavailableBenchmarks.length > FORECAST_RECONCILIATION_MAX_ROWS || value.audit.length > FORECAST_RECONCILIATION_MAX_ROWS || value.audit.length > FORECAST_SHADOW_EPOCH_MAX_AUDITS || value.scores.length + value.pending.length + value.dataGaps.length + value.unavailableBenchmarks.length > FORECAST_RECONCILIATION_MAX_ROWS) issues.push({ code: "row-limit", path: "index", message: "Reconciliation rows exceed the v1 or bounded-epoch limits." });
    ...
    value.unavailableBenchmarks.forEach((entry, index) => issues.push(...unavailableBenchmarkEntryIssues(entry, `index.unavailableBenchmarks[${index}]`)));
    ...
    const unavailable = value.unavailableBenchmarks as ForecastReconciliationUnavailableBenchmarkEntryV1[];
    ...
    if (stableSerializeHistoricalAnalysis(unavailable) !== stableSerializeHistoricalAnalysis([...unavailable].sort((a, b) => textOrder(statusKey(a), statusKey(b)))) || new Set(unavailable.map(statusKey)).size !== unavailable.length) issues.push({ code: "invalid-order", path: "index.unavailableBenchmarks", message: "Unavailable benchmarks must be unique and in canonical order." });
    ...
    const unavailableKeys = unavailable.map(statusKey);
    if (new Set([...allKeys, ...unavailableKeys]).size !== allKeys.length + unavailableKeys.length) issues.push({ code: "invalid-row", path: "index", message: "Each benchmark prediction can have exactly one available-state or unavailable-origin row." });
    if (allKeys.length + unavailableKeys.length > FORECAST_SHADOW_EPOCH_MAX_TARGETS) issues.push({ code: "row-limit", path: "index", message: "The evaluation epoch benchmark-row limit was exceeded." });
    const referencedForecasts = sortedUnique([...scores, ...pending, ...gaps, ...unavailable, ...audit].map((entry) => entry.forecastArtifactId));
    ...
    if ([...scores, ...pending, ...gaps, ...unavailable, ...audit].some((entry) => sourceForecastById.get(entry.forecastArtifactId)?.generatedAt !== entry.forecastGeneratedAt)) issues.push({ code: "incompatible-artifact", path: "index", message: "Every state and audit row must retain its sampled forecast generation instant." });
    ...
    if (allKeys.length + unavailableKeys.length >= FORECAST_SHADOW_EPOCH_MAX_TARGETS) activeStopReasons.add("target-row-limit-reached");
    ...
    if (forecastArtifacts && Array.isArray(value.sourceForecastArtifactIds)) {
      ...
      const expectedAvailableKeys = sortedUnique(sourceIds.flatMap((id) => {
        const forecast = forecastArtifacts.get(id);
      ...
      const expectedUnavailableKeys = sortedUnique(sourceIds.flatMap((id) => {
        const forecast = forecastArtifacts.get(id);
        return (forecast?.targets ?? [])
          .filter((target) => target.availability === "available")
          .flatMap((target) =>
            target.benchmarks
              .filter((benchmark) => benchmark.availability === "unavailable")
              .map((benchmark) =>
                statusKey({
                  forecastArtifactId: id,
                  targetKind: target.targetKind,
      ...
      if (stableSerializeHistoricalAnalysis(expectedAvailableKeys) !== stableSerializeHistoricalAnalysis(actualKeys)) issues.push({ code: "incompatible-artifact", path: "index", message: "Index must contain exactly one state row for every available benchmark prediction in the complete source forecast set." });
      if (stableSerializeHistoricalAnalysis(expectedUnavailableKeys) !== stableSerializeHistoricalAnalysis(unavailable.map(statusKey))) issues.push({ code: "incompatible-artifact", path: "index.unavailableBenchmarks", message: "Index must contain exactly one unavailable row for every unavailable origin benchmark in the complete source forecast set." });
      ...
      for (const id of sourceIds) for (const target of forecastArtifacts.get(id)?.targets ?? []) {
        ...
        const forecast = forecastArtifacts.get(id)!;
        for (const benchmark of target.benchmarks) {
          const expected = targetBase(target, forecast, benchmark);
          const actual = entriesByKey.get(statusKey(expected));
          const unavailableEntry = unavailable.find(
            (entry) => statusKey(entry) === statusKey(expected),
          );
          if (benchmark.availability === "available") {
            if (!actual || stableSerializeHistoricalAnalysis(entryBase(actual)) !== stableSerializeHistoricalAnalysis(expected)) issues.push({ code: "incompatible-artifact", path: "index", message: "Every available benchmark state must retain its exact immutable target snapshot from the source forecast." });
          } else if (!unavailableEntry || stableSerializeHistoricalAnalysis(entryBase(unavailableEntry)) !== stableSerializeHistoricalAnalysis(expected) || unavailableEntry.reason !== benchmark.reason) {
            issues.push({ code: "incompatible-artifact", path: "index.unavailableBenchmarks", message: "Every unavailable benchmark must retain its exact origin-time reason and immutable target snapshot." });
          }
        }
      }
    }
...
type AvailableForecastBenchmark = Extract<
  ForecastArtifactBenchmarkV1,
  { availability: "available" }
>;

function forecastBenchmarkFingerprint(
  benchmark: ForecastArtifactBenchmarkV1,
): string {
  return historicalAnalysisFingerprint(benchmark);
}

function benchmarkCohortId(benchmark: ForecastArtifactBenchmarkV1): string {
  return `${benchmark.benchmarkId}:${historicalAnalysisFingerprint({
    cohorts: benchmark.cohorts,
  })}`;
}

function benchmarkPointEstimator(
  target: Extract<ForecastArtifactTargetV1, { availability: "available" }>,
  benchmark: AvailableForecastBenchmark,
): ForecastScorePointEstimatorV1 {
  if (benchmark.benchmarkId === "selected-private-model") {
    return target.prediction.pointEstimator;
  }
  if (benchmark.benchmarkId === "current-public-heuristic") {
    return "current-public-heuristic";
  }
  return target.targetKind === "public-release"
    ? "platform-stage-median"
    : "next-event-simple-baseline";
}

/**
 * The selected benchmark is the artifact's primary calibrated prediction, so
 * its complete frozen 50%/80% interval pair remains scoreable. Comparator
 * rows own only the empirical range they explicitly froze at origin.
 */
function benchmarkIntervalBounds(
  target: Extract<ForecastArtifactTargetV1, { availability: "available" }>,
  benchmark: AvailableForecastBenchmark,
): readonly Pick<ForecastScoreIntervalV1, "level" | "lowerDays" | "upperDays">[] {
  if (benchmark.benchmarkId === "selected-private-model") {
    return target.prediction.intervals.map((interval) => ({
      level: interval.level,
      lowerDays: interval.lowerDays,
      upperDays: interval.upperDays,
    }));
  }
  const range = benchmark.prediction.empiricalRange;
  return range
    ? [{ level: range.level, lowerDays: range.lowerDays, upperDays: range.upperDays }]
    : [];
}
...
function targetBase(
  ...
  benchmark: ForecastArtifactBenchmarkV1,
): ForecastReconciliationEntryBaseV1 {
  return {
    ...
    modelCohortId: benchmarkCohortId(benchmark),
    targetSnapshot: {
      forecastTargetFingerprint: historicalAnalysisFingerprint(target),
      forecastBenchmarkFingerprint: forecastBenchmarkFingerprint(benchmark),
      ...
      predictedEligibleStage:
        ...
        benchmark.prediction.targetKind === "next-eligible-prerelease-event"
          ? benchmark.prediction.predictedEligibleStage
          : null,
    },
  };
}
...
function scoreFromOutcome(
  forecast: ForecastArtifactV1,
  target: Extract<ForecastArtifactTargetV1, { availability: "available" }>,
  benchmark: AvailableForecastBenchmark,
  outcome: ForecastOutcomeObservationV1,
): ForecastScoreArtifactV1 {
  const actualDays = elapsedDays(target.anchorOccurredOn, outcome.occurredOn);
  const signedErrorDays = normalizeZero(actualDays - benchmark.prediction.pointDays);
  const intervals = benchmarkIntervalBounds(target, benchmark).map((interval) => ({
    ...interval,
    covered: actualDays >= interval.lowerDays && actualDays <= interval.upperDays,
  }));
  const score: ForecastScoreArtifactV1 = {
    ...
    benchmarkId: benchmark.benchmarkId,
    forecastBenchmarkFingerprint: forecastBenchmarkFingerprint(benchmark),
    forecastBenchmarkSourceFingerprint: benchmark.sourceFingerprint,
    targetId: target.targetId,
    targetKind: target.targetKind,
    releaseId: target.releaseId,
    platformId: target.platformId,
    ...
    modelFingerprint: benchmark.modelFingerprint,
    calibrationFingerprint: benchmark.calibrationFingerprint,
    modelCohortId: benchmarkCohortId(benchmark),
    predictedEligibleStage:
      benchmark.prediction.targetKind === "next-eligible-prerelease-event"
        ? benchmark.prediction.predictedEligibleStage
    ...
    actualDays,
    pointEstimator: benchmarkPointEstimator(target, benchmark),
    pointDays: benchmark.prediction.pointDays,
    signedErrorDays,
...
function scoreEntryFromArtifact(base: ForecastReconciliationEntryBaseV1, scoreArtifactId: string, score: ForecastScoreArtifactV1): ForecastReconciliationScoreEntryV1 {
  return {
    ...
    coverage50:
      score.intervals.find((interval) => interval.level === 0.5)?.covered ??
      null,
    coverage80:
      score.intervals.find((interval) => interval.level === 0.8)?.covered ??
      null,
  };
}
...
export function validateForecastScoreAgainstForecast(score: ForecastScoreArtifactV1, forecast: ForecastArtifactV1): ForecastScoringValidationIssue[] {
  ...
  const benchmark = target.benchmarks.find(
    (candidate) => candidate.benchmarkId === score.benchmarkId,
  );
  if (!benchmark || benchmark.availability !== "available") {
    return [
      ...issues,
      {
        code: "incompatible-artifact",
        path: "score.benchmarkId",
        message: "Scores may bind only an available immutable origin benchmark.",
      },
    ];
  }
  if (target.releaseId !== score.releaseId || target.platformId !== score.platformId || target.anchorEventId !== score.anchorEventId || target.anchorStage !== score.anchorStage || target.anchorOccurredOn !== score.anchorOccurredOn || target.originOn !== score.originOn || benchmark.modelVersion !== score.modelVersion || benchmark.modelFingerprint !== score.modelFingerprint || benchmark.calibrationFingerprint !== score.calibrationFingerprint || benchmark.sourceFingerprint !== score.forecastBenchmarkSourceFingerprint || forecastBenchmarkFingerprint(benchmark) !== score.forecastBenchmarkFingerprint || benchmarkCohortId(benchmark) !== score.modelCohortId || benchmarkPointEstimator(target, benchmark) !== score.pointEstimator || benchmark.prediction.pointDays !== score.pointDays) issues.push({ code: "incompatible-artifact", path: "score.benchmarkId", message: "Score benchmark identity, estimator, cohort, source, or prediction differs from the immutable origin row." });
  ...
  const predictedStage = benchmark.prediction.targetKind === "next-eligible-prerelease-event" ? benchmark.prediction.predictedEligibleStage : null;
  ...
  const expectedIntervals = benchmarkIntervalBounds(target, benchmark);
  if (score.intervals.length !== expectedIntervals.length) {
    issues.push({ code: "incompatible-artifact", path: "score.intervals", message: "Score interval availability differs from the immutable benchmark." });
  }
  for (const [index, interval] of expectedIntervals.entries()) {
...
function prepareForecastScoreReconciliation(args: ReconcileForecastScoresArgs): PreparedReconciliation {
  ...
  const unavailableBenchmarks = new Map<string, ForecastReconciliationUnavailableBenchmarkEntryV1>(
    (previousIndex?.unavailableBenchmarks ?? []).map((entry) => [
      statusKey(entry),
      entry,
    ]),
  );
  ...
  for (const forecast of args.forecastArtifacts) {
    const availableTargets = forecast.targets.filter(
      (target) => target.availability === "available",
    );
    const availableBenchmarkRows = availableTargets.flatMap(
      (target) => target.benchmarks,
    );
    if (availableTargets.length > FORECAST_SHADOW_EPOCH_MAX_TARGETS_PER_FORECAST) {
      issues.push({ code: "row-limit", path: "forecastArtifacts", message: "One sampled forecast exceeds the fixed per-run target bound." });
    }
    if (
      availableBenchmarkRows.length >
      FORECAST_SHADOW_EPOCH_MAX_BENCHMARK_ROWS_PER_FORECAST
    ) {
      issues.push({ code: "row-limit", path: "forecastArtifacts", message: "One sampled forecast exceeds the fixed per-run benchmark-row bound." });
    }
  ...
  for (const forecast of resolvedForecasts) {
    ...
    for (const target of forecast.targets) {
      if (target.availability !== "available") continue;
      for (const benchmark of target.benchmarks) {
        const base = targetBase(target, forecast, benchmark);
        const key = statusKey(base);
        expectedKeys.push(key);
        if (benchmark.availability === "available") {
          const existing = contexts.get(key);
          if (artifactIsPrior && (!existing || stableSerializeHistoricalAnalysis(existing.base) !== stableSerializeHistoricalAnalysis(base))) issues.push({ code: "incompatible-artifact", path: "previousForecastArtifacts", message: "A resolved prior forecast must match every immutable available benchmark snapshot in the prior index." });
          contexts.set(key, { base, forecast, target, benchmark });
          unavailableBenchmarks.delete(key);
        } else {
          const unavailable = { ...base, reason: benchmark.reason };
          const existing = unavailableBenchmarks.get(key);
          if (artifactIsPrior && (!existing || stableSerializeHistoricalAnalysis(existing) !== stableSerializeHistoricalAnalysis(unavailable))) issues.push({ code: "incompatible-artifact", path: "previousForecastArtifacts", message: "A resolved prior forecast must retain every unavailable benchmark and its origin-time reason exactly." });
          unavailableBenchmarks.set(key, unavailable);
          contexts.delete(key);
        }
      }
    }
    if (artifactIsPrior) {
      const actualKeys = [
        ...priorStates.filter((entry) => entry.forecastArtifactId === forecast.artifactId).map(statusKey),
        ...(previousIndex?.unavailableBenchmarks ?? [])
          .filter((entry) => entry.forecastArtifactId === forecast.artifactId)
          .map(statusKey),
      ].sort(textOrder);
  ...
  if (contexts.size + unavailableBenchmarks.size > FORECAST_RECONCILIATION_MAX_ROWS) issues.push({ code: "row-limit", path: "index", message: "The complete benchmark union exceeds the bounded reconciliation capacity." });
  if (contexts.size + unavailableBenchmarks.size > FORECAST_SHADOW_EPOCH_MAX_TARGETS) issues.push({ code: "row-limit", path: "index", message: "The fixed evaluation epoch benchmark-row limit was exceeded." });
  if (issues.length) throw new ForecastScoringContractError(issues);
  ...
  return {
    ...
    derivedByKey: derivedOutcomes,
    sourceForecastArtifactIds,
    sourceForecasts,
    unavailableBenchmarks: [...unavailableBenchmarks.values()].sort((left, right) =>
      textOrder(statusKey(left), statusKey(right)),
    ),
    requiredForecastArtifactIds,
  };
}
...
export function reconcileForecastScores(args: ReconcileForecastScoresArgs): ForecastScoreReconciliationResultV1 {
  ...
  for (const context of prepared.contexts) {
      ...
      if ("outcome" in derived) {
        ...
        if (!context.forecast || !context.target || !context.benchmark) throw new ForecastScoringContractError([{ code: "incompatible-artifact", path: "previousForecastArtifacts", message: "A score-producing transition requires its exact content-addressed forecast and available benchmark." }]);
        const nextScore = scoreFromOutcome(
          context.forecast,
          context.target,
          context.benchmark,
          derived.outcome,
  ...
  const draft = { indexVersion: FORECAST_RECONCILIATION_INDEX_VERSION, mode: FORECAST_ARTIFACT_MODE, compatibleForecastArtifactVersion: FORECAST_ARTIFACT_VERSION, compatibleScoreVersion: FORECAST_SCORE_VERSION, reconciliationCutoffAt: args.reconciliationCutoffAt, reconciliationCutoffDate: args.reconciliationCutoffAt.slice(0, 10), evaluationEpoch: args.evaluationEpoch, epochStopReason: null, sourceForecastArtifactIds: prepared.sourceForecastArtifactIds, sourceForecasts: prepared.sourceForecasts, scores, pending, dataGaps, unavailableBenchmarks: prepared.unavailableBenchmarks, audit } satisfies Omit<ForecastReconciliationIndexV1, "indexFingerprint">;
  ...
  const currentStopReason: ForecastShadowEpochStopReason | null = prepared.sourceForecasts.length >= FORECAST_SHADOW_EPOCH_MAX_FORECASTS
    ...
    : prepared.contexts.length + prepared.unavailableBenchmarks.length >= FORECAST_SHADOW_EPOCH_MAX_TARGETS
...
function addHealthStatus(groups: Map<string, HealthAccumulator>, entry: ForecastReconciliationEntryBaseV1, status: "score" | "pending" | "gap", score?: ForecastReconciliationScoreEntryV1): void {
  ...
  for (const dimension of dimensions) {
    ...
    const key = `${entry.targetKind}\u0000${entry.benchmarkId}\u0000${dimension.groupKind}\u0000${dimension.groupId}`;
    const accumulator = groups.get(key) ?? { targetKind: entry.targetKind, benchmarkId: entry.benchmarkId, ...dimension, forecastCount: 0, pendingCount: 0, dataGapCount: 0, scores: [] };
...
function metricFromAccumulator(group: HealthAccumulator): ForecastShadowHealthMetricV1 {
  ...
  for (const score of group.scores) {
    const key = `${score.targetKind}\u0000${score.benchmarkId}\u0000${score.platformId}\u0000${score.outcomeEventId}\u0000${score.outcomeOccurredOn}`;
  ...
  const eventSummaries = [...eventGroups.entries()].sort(([left], [right]) => textOrder(left, right)).map(([, scores]) => ({
    ...
    coverage50: (() => {
      const intervalScores = scores.filter(
        (score): score is ForecastReconciliationScoreEntryV1 & { coverage50: boolean } =>
          score.coverage50 !== null,
      );
      return intervalScores.length === 0
        ? null
        : intervalScores.filter((score) => score.coverage50).length /
          intervalScores.length;
    })(),
    coverage80: (() => {
      const intervalScores = scores.filter(
        (score): score is ForecastReconciliationScoreEntryV1 & { coverage80: boolean } =>
          score.coverage80 !== null,
      );
      return intervalScores.length === 0
        ? null
        : intervalScores.filter((score) => score.coverage80).length /
          intervalScores.length;
    })(),
  }));
  const realizedEventCount = eventSummaries.length;
  const coverage50Values = eventSummaries.flatMap((event) =>
    event.coverage50 === null ? [] : [event.coverage50],
  );
  const coverage80Values = eventSummaries.flatMap((event) =>
    event.coverage80 === null ? [] : [event.coverage80],
  );
  const base: ForecastShadowHealthMetricV1Base = {
    ...
    coverage50EventCount: coverage50Values.length,
    coverage80EventCount: coverage80Values.length,
  };
  ...
  return {
    ...
    coverage50: coverage50Values.length === 0
      ? null
      : coverage50Values.reduce((sum, coverage) => sum + coverage, 0) /
        coverage50Values.length,
    coverage80: coverage80Values.length === 0
      ? null
      : coverage80Values.reduce((sum, coverage) => sum + coverage, 0) /
        coverage80Values.length,
  };
}
...
export function validateForecastShadowHealthReport(value: unknown): ForecastScoringValidationIssue[] {
  try {
    ...
    if (!isRecord(value.summary)) issues.push({ code: "invalid-row", path: "health.summary", message: "Health summary is required." });
      ...
      exactKeys(value.summary, ["forecastCount", "unavailableBenchmarkCount", "scoredCount", "pendingCount", "dataGapCount", "runFailureCount"], "health.summary", issues);
      if (![value.summary.forecastCount, value.summary.unavailableBenchmarkCount, value.summary.scoredCount, value.summary.pendingCount, value.summary.dataGapCount, value.summary.runFailureCount].every(isCount) || (isCount(value.summary.forecastCount) && isCount(value.summary.scoredCount) && isCount(value.summary.pendingCount) && isCount(value.summary.dataGapCount) && value.summary.forecastCount !== value.summary.scoredCount + value.summary.pendingCount + value.summary.dataGapCount)) issues.push({ code: "invalid-row", path: "health.summary", message: "Summary counts must be non-negative and reconcile exactly." });
    }
    ...
    value.metrics.forEach((metric, index) => {
      ...
      exactKeys(metric, available ? ["targetKind", "benchmarkId", "groupKind", "groupId", "forecastCount", "scoredCount", "realizedEventCount", "pendingCount", "dataGapCount", "scoreCoverage", "coverage50EventCount", "coverage80EventCount", "availability", "meanAbsoluteErrorDays", "medianAbsoluteErrorDays", "signedBiasDays", "coverage50", "coverage80"] : ["targetKind", "benchmarkId", "groupKind", "groupId", "forecastCount", "scoredCount", "realizedEventCount", "pendingCount", "dataGapCount", "scoreCoverage", "coverage50EventCount", "coverage80EventCount", "availability", "reason"], path, issues);
      if (!["public-release", "next-eligible-prerelease-event"].includes(metric.targetKind as string) || !BENCHMARK_IDS.has(metric.benchmarkId as ForecastBenchmarkIdV1) || !["overall", "platform", "model-cohort"].includes(metric.groupKind as string) || !isText(metric.groupId, 256) || ![metric.forecastCount, metric.scoredCount, metric.realizedEventCount, metric.pendingCount, metric.dataGapCount, metric.coverage50EventCount, metric.coverage80EventCount].every(isCount) || (isCount(metric.forecastCount) && metric.forecastCount < 1) || (isCount(metric.realizedEventCount) && isCount(metric.scoredCount) && metric.realizedEventCount > metric.scoredCount) || (isCount(metric.coverage50EventCount) && isCount(metric.realizedEventCount) && metric.coverage50EventCount > metric.realizedEventCount) || (isCount(metric.coverage80EventCount) && isCount(metric.realizedEventCount) && metric.coverage80EventCount > metric.realizedEventCount) || !isFiniteNumber(metric.scoreCoverage) || metric.scoreCoverage < 0 || metric.scoreCoverage > 1 || (isCount(metric.forecastCount) && isCount(metric.scoredCount) && isCount(metric.pendingCount) && isCount(metric.dataGapCount) && (metric.forecastCount !== metric.scoredCount + metric.pendingCount + metric.dataGapCount || metric.scoreCoverage !== metric.scoredCount / metric.forecastCount))) issues.push({ code: "invalid-row", path, message: "Metric benchmark identity, state counts, interval denominators, and unique realized-event counts must be exact." });
      if (available) {
        const coverage50Valid = isCount(metric.coverage50EventCount) && (metric.coverage50EventCount === 0 ? metric.coverage50 === null : isFiniteNumber(metric.coverage50) && metric.coverage50 >= 0 && metric.coverage50 <= 1);
        const coverage80Valid = isCount(metric.coverage80EventCount) && (metric.coverage80EventCount === 0 ? metric.coverage80 === null : isFiniteNumber(metric.coverage80) && metric.coverage80 >= 0 && metric.coverage80 <= 1);
        if (!isCount(metric.realizedEventCount) || metric.realizedEventCount < FORECAST_HEALTH_MIN_REPORTABLE_SCORES || ![metric.meanAbsoluteErrorDays, metric.medianAbsoluteErrorDays, metric.signedBiasDays].every(isFiniteNumber) || (isFiniteNumber(metric.meanAbsoluteErrorDays) && metric.meanAbsoluteErrorDays < 0) || (isFiniteNumber(metric.medianAbsoluteErrorDays) && metric.medianAbsoluteErrorDays < 0) || !coverage50Valid || !coverage80Valid || (isFiniteNumber(metric.coverage50) && isFiniteNumber(metric.coverage80) && metric.coverage50 > metric.coverage80)) issues.push({ code: "invalid-row", path, message: "Reportable metrics require eight unique realized events and exact interval-availability-aware coverage." });
    ...
    for (const kind of ["public-release", "next-eligible-prerelease-event"] as const) {
      for (const benchmarkId of FORECAST_BENCHMARK_IDS) {
        const root = overall.find(
          (metric) =>
            metric.targetKind === kind && metric.benchmarkId === benchmarkId,
        );
        for (const groupKind of ["platform", "model-cohort"] as const) {
          const rows = metrics.filter(
            (metric) =>
              metric.targetKind === kind &&
              metric.benchmarkId === benchmarkId &&
              metric.groupKind === groupKind,
          );
          if (
            root &&
            metricCountFields.some(
              (field) =>
                rows.reduce((sum, row) => sum + row[field], 0) !== root[field],
            )
          ) {
            issues.push({ code: "invalid-row", path: "health.metrics", message: "Platform and cohort metrics must each partition their target-kind/benchmark overall row exactly." });
          }
        }
      }
    }
...
export function buildForecastShadowHealthReport(args: {
  ...
  const withoutFingerprint: Omit<ForecastShadowHealthReportV1, "reportFingerprint"> = {
    ...
    summary: { forecastCount, unavailableBenchmarkCount: args.index.unavailableBenchmarks.length, scoredCount: args.index.scores.length, pendingCount: args.index.pending.length, dataGapCount: args.index.dataGaps.length, runFailureCount: runFailures.length },
    dataGapCounts,
    metrics,
  };
...
export function renderForecastShadowHealthReport(report: ForecastShadowHealthReportV1): string {
  ...
  const lines = [
    ...
    `Forecasts: ${report.summary.forecastCount}; unavailable benchmark rows: ${report.summary.unavailableBenchmarkCount}; scored: ${report.summary.scoredCount}; pending: ${report.summary.pendingCount}; data gaps: ${report.summary.dataGapCount}; run failures: ${report.summary.runFailureCount}`,
    "",
    "Model performance (data gaps excluded):",
  ];
  for (const metric of report.metrics.filter((entry) => entry.groupKind === "overall")) {
    const coverage50 = metric.availability === "available" && metric.coverage50 !== null
      ? `${(metric.coverage50 * 100).toFixed(1)}%`
      : "n/a";
    const coverage80 = metric.availability === "available" && metric.coverage80 !== null
      ? `${(metric.coverage80 * 100).toFixed(1)}%`
      : "n/a";
    lines.push(metric.availability === "available"
      ? `- ${metric.targetKind}/${metric.benchmarkId}: forecasts=${metric.scoredCount}, unique events=${metric.realizedEventCount}, event-weighted MAE=${metric.meanAbsoluteErrorDays.toFixed(2)}d, bias=${metric.signedBiasDays.toFixed(2)}d, 50%=${coverage50}, 80%=${coverage80}`
      : `- ${metric.targetKind}/${metric.benchmarkId}: forecasts=${metric.scoredCount}, unique events=${metric.realizedEventCount}; metrics withheld until ${FORECAST_HEALTH_MIN_REPORTABLE_SCORES} unique events`);
  }
